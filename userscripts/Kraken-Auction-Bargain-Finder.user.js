...
(function () {
  ...
  function makePanel(panel) {
    ...
    $('kabf-weapon-slot').addEventListener('change',async e=>{state.weaponSlot=e.target.value;pruneWeaponSelections();refreshWeaponOptions();refreshBonusOptions();renderResults();if(!state.busy){await loadMatchingHistories();renderResults();}});
  ...
  function pruneWeaponSelections(){if(state.activeMode!=='weapon')return;const itemNames=new Set(state.weaponSlot==='all'?state.weaponCatalogue:state.weaponCatalogues[state.weaponSlot]||[]),bonusNames=new Set(eligibleBonuses());state.selections.weapon.items=state.selections.weapon.items.filter(x=>itemNames.has(x));state.selections.weapon.bonuses=state.selections.weapon.bonuses.filter(x=>bonusNames.has(x));}
  function refreshWeaponOptions(){const box=$('kabf-weapon-options');if(!box||!state.activeMode)return;const selected=new Set(state.selections[state.activeMode].items),catalogue=state.activeMode==='armor'?state.armorCatalogue:(state.weaponSlot==='all'?state.weaponCatalogue:state.weaponCatalogues[state.weaponSlot]||[]),visibleRows=[...state.rows.values()].filter(r=>r.itemType===state.activeMode&&(state.activeMode==='armor'||state.weaponSlot==='all'||catalogue.includes(r.name))).map(r=>r.name),names=[...new Set([...catalogue,...visibleRows].filter(Boolean))];selected.forEach(x=>{if(!names.includes(x))names.push(x);});names.sort((a,b)=>a.localeCompare(b));box.innerHTML=names.map(x=>`<label><input type="checkbox" value="${escapeHtml(x)}" ${selected.has(x)?'checked':''}>${escapeHtml(x)}</label>`).join('');updatePickerSummaries();}
